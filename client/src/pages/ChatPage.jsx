...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    const socket = getSocket();
    if (socket) {
      socket.on('new_message', (message) => { alert('Me: ' + (user?.id||'null') + ' vs ' + message.senderId); console.log('NOTIF CHECK:', Notification.permission, message.senderId, user.id); if (Notification.permission === 'granted') { navigator.serviceWorker.ready.then(reg => reg.showNotification('New Dharma Message', { body: message.content, icon: '/vite.svg', vibrate: [200] })); }
